...
export class SkewNormal {
  ...
  public static random(location: number = 0,
                       scale: number = 1.0,
                       shape: number = 0,
                       min: number = -Infinity,
                       max: number = Infinity): number {
    let sigma: number, u0: number, v: number, u1: number, ret: number;
  ...
  public static pdf(location: number = 0, scale: number = 1.0, shape: number = 0, x: number = 0): number {
    const exp = Math.exp(-((x - location) * (x - location)) / (2 * scale * scale));
    const err = ErrorFunction.erfc(-(shape * (x - location)) / (SkewNormal.sqrtTwo * scale));
  ...
  public static expectedValue(location: number = 0, scale: number = 1.0, shape: number = 0) {
    const ro = shape / Math.sqrt(1 + shape * shape);
